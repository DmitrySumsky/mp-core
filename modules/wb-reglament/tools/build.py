# -*- coding: utf-8 -*-
"""Сборка центрального кода модуля «Регламент WB» со встроенными проверками.

v1.0.0 (25.09.2026): по образцу wb-search-positions; вызовы без shell=True — на macOS
shell=True со списком аргументов запускал голый `node`, и `node --check` не проверял ничего.
...
PUBLIC = ["updateWbReglament", "updateWbSellerWarehouses", "fbsApplyPlan", "fbsSetToken",
          "rgAddNewItems", "rgInstallTriggers", "rgRemoveTriggers", "upgradeSheets",
          "rgCheckConnection", "rgHelp", "rgStatus", "wbDailyTrigger", "wbWarehousesDailyTrigger"]

# Внутренние функции, без которых прогон молча теряет этап
REQUIRED = ["wbUpdateReglamentLocked_", "wbFetchStockProducts_", "wbFetchRfWarehouseStockProducts_",
            "wbFetchBarcodeToNm_", "fbsRunLocked_", "fbsPlanOne_", "fbsPutStocks_", "fbsLog_",
            "fbsConfirm_", "installWbDailyTriggerSilent", "installWbWarehousesDailyTriggerSilent"]
...
def brand_words():
    ...
    path = os.environ.get("RG_LEAK_WORDS", "")
...
def check(text):
    ...
    bad = []
    tmp = os.path.join(tempfile.gettempdir(), "rg_central_check.js")
    ...
    for label, body in (("central", text), ("loader", loader)):
        ...
        # границы слова: «upgrade» в имени upgradeSheets — не бренд (грабля соседнего модуля)
