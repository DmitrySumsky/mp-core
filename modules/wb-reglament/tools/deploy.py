# -*- coding: utf-8 -*-
"""Деплой лоадера в привязанный скрипт книги (clasp). Модуль «Регламент WB».

v1.0.0 (25.09.2026): по образцу wb-search-positions; без shell=True (на macOS он ломал вызовы);
переменные окружения RG_BOOKS_REGISTRY и RG_LEAK_WORDS.

Идентификаторы книг в открытом репозитории не хранятся: реестр лежит ВНЕ репозитория,
путь — аргумент --registry или переменная RG_BOOKS_REGISTRY. Формат файла:

    {"books": [{"key": "reglament-1", "title": "...", "sheet_id": "...", "script_id": "..."}]}
...
def registry_path():
    ...
    return os.environ.get("RG_BOOKS_REGISTRY", "")


def books():
    path = registry_path()
    if not path or not os.path.exists(path):
        print("нет реестра книг: --registry <файл> или переменная RG_BOOKS_REGISTRY")
        raise SystemExit(1)
    return json.load(io.open(path, encoding="utf-8"))["books"]


def deploy(book):
    work = tempfile.mkdtemp(prefix="rg_deploy_")
    try:
