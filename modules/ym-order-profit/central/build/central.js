/* ЧП ПО ЗАКАЗАМ ЯНДЕКС МАРКЕТА — ЦЕНТРАЛЬНЫЙ КОД v2.4.0 — 25.09.2026 */
/*
 ...
 *   08_примечания.js — примечания к заголовкам: что это, откуда, формула
 *
 * ИСТОРИЯ ВЕРСИЙ (новая сверху)
 *
 * v2.4.0 — 25.09.2026
 *   «ЧТОБЫ В ПРИМЕЧАНИЯХ БЫЛО РАСПИСАНО, КАК СЧИТАЕТСЯ ПОКАЗАТЕЛЬ, ПРЯМ С ФОРМУЛАМИ» — владелец, 25.09.2026, как в РНП Ozon;
 *   до этого менеджер ЯМ спрашивала по колонкам, что фактом из Маркета, а что по коэффициентам.
 *   • 08_примечания.js: у заголовка каждой колонки «юнитки», «по заказам» и «по дням» — примечание «Что / Откуда / Формула»;
 *     в юнитке буквы колонок в формулах берутся из YOP_UNIT_SPEC, при сдвиге колонок примечания сдвигаются сами;
 *   • примечания пишет каждый прогон; сбой записи примечаний прогон не роняет;
 *   • «как считается» и шапка юнитки: «наведите на заголовок колонки».
 *   Тесты: 32/32 + 10/10.
 *
 * v2.3.0 — 25.09.2026
 ...
 *   • юнитка: «Минимум для акции» (offer-prices, minimumForBestseller), «Цена в акции» (promoPrice), «Макс. акц. цена»
 *     (maxPromoPrice) и «Акция: участие» (promos + promos/offers) — как на старых листах акций книги;
...
function yopWriteDetail_(day, all) {
  ...
  yopPutNotes_(sh, YOP_DETAIL_NOTES, YOP_DETAIL_HEAD);                            // v2.4.0: что это, откуда, формула — при наведении
  if (rows.length) {
...
function yopWriteUnit_(day, all) {
  ...
  yopTitle_(sh, 'Юнитка ЯМ на данных Маркета на ' + yopRu_(day) + ' — экономика одной выкупленной штуки (' + YOP_VERSION + ')',
    'Наведите на заголовок колонки — в примечании что это, откуда цифра и формула. ' +
  ...
  yopPutNotes_(sh, yopUnitNotes_(), YOP_UNIT_HEAD);
  if (rows.length) {
    var n = rows.length, R = YOP_HEAD_ROW + 1;
...
var YOP_HELP_BOLD = ['Главное правило', 'Что фактом из Маркета, а что прогнозом', 'Откуда коэффициенты', 'Налог',
  'Факт рядом с прогнозом', 'Юнитка на данных Маркета', 'Себестоимость', 'Смена тарифа комиссии', 'Новый кабинет', 'Как проверить цифру',
  'Где код'];
...
function yopWriteHelp_() {
  var L = [
    ...
    ['Как проверить цифру'],
    ['Наведите на заголовок колонки на листах «' + YOP_SH.unit + '», «' + YOP_SH.detail + '» и «' + YOP_SH.days + '»: в примечании ' +
     'что это за показатель, откуда он берётся (API или отчёт Маркета) и формула с буквами колонок.'], [''],
    ['Где код'],
...
function yopWriteDay_(day, all) {
  ...
  yopPutNotes_(sh, YOP_DAYS_NOTES, YOP_DAYS_HEAD);
  SpreadsheetApp.flush();
